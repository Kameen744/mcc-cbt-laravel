    ...
    <div class="container-fluid" v-if="!examStart">
      ...
      <div class="jumbotron jumbotron-fluid">
          <div class="container">
            ...
            <h3 v-if="timeHasFinish">
                Your time on this exam has finished.
            </h3>
            <p class="lead" v-else>
                <a class="btn btn-primary btn-lg" href="" :class="{'disabled' : !startExamButton}" 
                @click.prevent="startExam">Start Exam</a>
            </p>
    ...
    <div class="container-fluid bg-gray-400" v-if="examStart">
      <nav class="navbar navbar-expand-sm navbar-light bg-gray-200 navbar-fixed-top shadow-sm py-0">
        <a class="navbar-brand" href="#">MCCHST CBT</a>
            ...
            <exam-timer class="ml-auto" :startTime="startTime" :endTime="endTime"
                v-on:timeEnd="timerEnds">
            </exam-timer>
      ...
      <div class="row mb-0" v-if="currentCourse">
          <div class="col-md-9">
                <div class="row d-flex justify-content-center m-0 p-0">
                    <button class="btn btn-flat btn-sm m-2" 
                    v-for="(course, key) in currentExam.course" :key="key" @click="getQuestions(key)"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <span v-for="(section, key) in questions[currentCourse.id].sections" :key="key">
                            <div class="row border-bottom-primary d-flex justify-content-between mb-1 mx-1"
                            v-if="section.id === questions[currentCourse.id].questions[currentQuestion].section_id">
                                <b class="">Question No: {{currentQuestion +1}} / {{questions[currentCourse.id].questions.length}}</b>
                                <b class="text-right">{{section.section}}</b>
                            </div>
                            <p class="text-center text-dark sectionQuestion p-2 bg-gray-300 shadow-sm" 
                            v-if="section.id === questions[currentCourse.id].questions[currentQuestion].section_id"
                ...
                <div class="row" v-for="(question, key) in questions[currentCourse.id].questions" :key="key">
                    ...
                    <div class="col-md-12" v-if="key === currentQuestion">
                        ...
                        <div class="list-group shadow-sm mt-0" :key="checked">
                            ...
                            <!-- <button class="list-group-item list-group-item-action bg-gray-300 border-0 shadow-sm d-flex py-2"
                            @click="attemptQuestion(question, 'A')">
                                <button class="btn-circle"
                                :class=" mrkAtt(question.id, 'A') ? 'btn-primary' : 'btn-sm btn-secondary' ">
                                    <b>A </b>
                                    <i :class="{ 'ml-1 fas fa-check' : mrkAtt(question.id, 'A') }"></i>
                                </button>
                                <b class="pl-2 pt-1" v-html="question.option_a"></b>
                            </button> -->

                            <!-- <button class="list-group-item list-group-item-action bg-gray-300 border-0 shadow-sm d-flex py-2"
                            @click="attemptQuestion(question, 'B')">
                                <button class="btn-circle"
                                :class=" mrkAtt(question.id, 'B') ? 'btn-primary' : 'btn-sm btn-secondary' ">
                                    <b>B </b>
                                    <i :class="{ 'ml-1 fas fa-check' : mrkAtt(question.id, 'B') }"></i>
                                </button>
                                <b class="pl-2 pt-1" v-html="question.option_b"></b>
                            </button>

                            <button class="list-group-item list-group-item-action bg-gray-300 border-0 shadow-sm d-flex py-2"
                            @click="attemptQuestion(question, 'C')">
                                <button class="btn-circle"
                                :class=" mrkAtt(question.id, 'C') ? 'btn-primary' : 'btn-sm btn-secondary' ">
                                    <b>C </b>
                                    <i :class="{ 'ml-1 fas fa-check' : mrkAtt(question.id, 'C') }"></i>
                                </button>
                                <b class="pl-2 pt-1" v-html="question.option_c"></b>
                            </button>

                            <button class="list-group-item list-group-item-action bg-gray-300 border-0 shadow-sm d-flex py-2"
                            @click="attemptQuestion(question, 'D')">
                                <button class="btn-circle"
                                :class=" mrkAtt(question.id, 'D') ? 'btn-primary' : 'btn-sm btn-secondary' ">
                                    <b>D </b>
                                    <i :class="{ 'ml-1 fas fa-check' : mrkAtt(question.id, 'D') }"></i>
                                </button>
                                <b class="pl-2 pt-1" v-html="question.option_d"></b>
                            </button> -->
              ...
              <div class="row bg-gray-200 shadow-sm p-2">
                  ...
                  <div class="col-md-12 mt-2">
                      <div class="row d-flex justify-content-between">
                          <b><i class="fas fa-circle fa-1x text-primary"></i> Attempted</b>
                          <b><i class="fas fa-circle fa-1x text-secondary"></i> Not Attempted</b>
                      </div>
                  </div>
                  <div class="col-12 mt-3">
                      <div class="row">
                          <button class="btn btn-secondary btn-sm btn-block mb-1 shadow-sm" 
                          @click="showCalculator"><i class="fas fa-calculator"></i> Calculator</button>
                          <button class="btn btn-secondary btn-sm btn-block mb-1 shadow-sm"><i class="fas fa-sticky-note    "></i> Instructions</button>
                          <button class="btn btn-danger btn-sm btn-block mt-2 shadow-sm">Finish</button>
                      </div>
                  </div>
              </div>
          </div>
      ...
      <hr class="m-0">
        <h6 class="text-left mb-1"><i class="fas fa-copyright"></i> MCCHST (Developed by Kamal 08034932065)</h6>
      </div>

        <!-- ---------------calculator modal----------------------- -->
        <div class="modal swal2-modal swal2-show" id="viewCalculator" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
...
export default {
    ...
    props: {
     ...
     created() {
        ...
        axios.get(`get_exams/${this.currentUser.department_id}`)
        .then(res => {
    ...
    methods: {
        ...
        getQuestions(courseKey) {
            ...
            if(!this.questions[crCourse.id]) {
                axios.get(`get_course_questions/${this.currentUser.id}/${this.currentExam.id}/${crCourse.id}`)
                .then(res => {
        ...
        getAttempted() {
            ...
            attemptsForm.post('get_attempted')
            .then(res => {
        ...
        setTotalNumberOfQuestions(crCourse) {
            this.currentQuestionsNumber = this.questions[crCourse.id].questions.length;
        },
        attemptQuestion(question, answer) {
            ...
            if(this.attemptForm.attempts.length === 5) {
                this.submitAttempt();
            }
        },
        submitAttempt() {
            this.attemptForm.post('attempt')
            .then(res => {
        ...
        nextQuestion() {
            if(this.currentQuestion < this.currentQuestionsNumber -1) {
                ...
                let curCourseIndex = this.currentExam.course.indexOf(this.currentCourse);
