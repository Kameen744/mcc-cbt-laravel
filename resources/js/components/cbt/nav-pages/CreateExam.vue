      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header py-1">
            <h5 v-show="!editExamMode" class="modal-title text-primary">
              New Exam
            </h5>
            <h5 v-show="editExamMode" class="modal-title text-warning">
              Edit Exam
            </h5>
            <button
              type="button"
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="container-fluid">
              ...
              <div
                class="card-body shadow-sm text-white bg-primary viewModalBody"
              >
                <h4 class="card-title text-center">{{ exam.exam }}</h4>
                <div class="row">
                  <p class="col-3">Date:</p>
                  <p class="col-8">{{ exam.exam_date | myDate }}</p>
                  <p class="col-3">Time:</p>
                  <p class="col-8">{{ exam.exam_time | myTime }}</p>
                  <p class="col-3">Duration:</p>
                  <p class="col-8">
                    {{ exam.exam_hours }}hr(s) {{ exam.exam_minutes }}min(s)
                  </p>
                  <div v-if="examSections[0]" class="col-12">
                    ...
                    >
                      ...
                      <tbody>
                        <tr
                          v-for="department in examDepartments"
                          :key="department.id"
                        >
              ...
              <div class="row">
                <div class="col-6">
                  ...
                  <div class="input-group">
                    <select
                      id="course"
                      @change="addExamCourse"
                      v-model="setCourseForm.course_id"
                      name="course_id"
                    >
                    ...
                    <select
                      data-toggle="tooltip"
                      title="Section"
                      v-model="setCourseForm.section_id"
                      id="section"
                    >
                      <option
                    ...
                    <input
                      ...
                      class="inp-sm"
                      type="number"
                      v-model="setCourseForm.no_questions"
                    />

                    <div class="input-group-btn">
                      <button
                        @click="addCourseSections"
                        data-toggle="tooltip"
                        title="Set course & no of questions"
                      >
                  ...
                  >
                    ...
                    <tbody>
                      <tr v-for="section in examSections" :key="section.id">
                        ...
                        <td>
                          <button
                            class="rounded-circle btn-danger"
                            @click="delExamSection(section.id)"
                          >
                ...
                <div class="col-md-6">
                  ...
                  <button
                    type="button"
                    class="btn"
                    :class="examStarted ? 'btn-success' : 'btn-danger'"
                    @click="startExam"
                  >
                    {{ examStarted ? "Exam Started" : "Start Exam" }}
                  </button>
                </div>
              </div>
              <hr />
              <button
                @click="settingExamModal(null)"
                class="btn btn-danger btn-sm"
              >
...
export default {
  ...
  methods: {
    ...
    getDeptExams(e) {
      this.$Progress.start();
      axios.get(`department/exam/${e.target.value}`).then((res) => {
        this.exams = res.data;
      });
    ...
    viewExam(exam) {
      ...
      axios
        ...
        .then(
          axios.spread((examSectionsRes, exmaDepartmentsRes) => {
    ...
    settingExamModal(exam) {
      if (exam) {
        ...
        axios
          ...
          .then(
            axios.spread((examSectionsRes, examDeptRes, coursesRes) => {
              this.examSections = examSectionsRes.data;
    ...
    addCourseSections() {
      this.setCourseForm.post(`exam/course_section`).then((res) => {
        this.examSections = res.data;
      });
    },
    addExamDepartments() {
      axios
        .post(`exam_set_dept/${this.exam.id}/${this.department_id}`)
        .then((res) => {
    ...
    editExam() {
      this.$Progress.start();
      this.form.put(`exam/${this.form.id}`).then((res) => {
        this.exams = res.data;
    ...
    delExamSection(id) {
      axios.get(`exam/del_exam_section/${id}`).then((res) => {
        this.examSections = res.data;
      });
    },
    delExamDepartment(id) {
      axios
        .get(`exam/del_exam_department/${this.exam.id}/${id}`)
        .then((res) => {
          this.examDepartments = res.data;
        });
    },
    ...
    startExam() {
      axios.get(`exam/start_exam/${this.exam.id}`).then((res) => {
        this.examStarted = res.data;
      });
    },
  },
};
