    ...
    <div class="card-body">
      <div class="row">
        <div class="col-md-12">
          ...
          <data-table :columns="columns" :key="data_table_key" :classes="classes" url="department"></data-table>
        </div>
      </div>
    </div>
    <div
            ...
            <div class="container-fluid">
              <div class="row shadow-sm justify-content-center">
                <div class="col-md-auto">
                  <button @click="showModal(department)" class="btn btn-info btn-sm">
                    <i class="fas fa-edit"></i> Edit
                  </button>
                </div>
                <div class="col-md-auto">
                  <button @click="delDeptartment(department.id)" class="btn btn-danger btn-sm">
...
export default {
  ...
  methods: {
    ...
    newDepartment() {
      this.$Progress.start();
      this.form.post(`department`).then(res => {
    ...
    editDepartment() {
      this.$Progress.start();
      this.form.put(`department/${this.form.id}`).then(res => {
        this.department = res.data;
        this.updateDataTable();
        Toast.fire({
          type: "success",
    ...
    delDeptartment(id) {
      Swal.fire({
        ...
        if (result.value) {
          axios.delete(`department/${id}`).then(res => {
            this.department = res.data;
            this.updateDataTable();
            $("#viewDept").modal("hide");
