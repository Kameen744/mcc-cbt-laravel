            ...
            <div class="container-fluid">
              <div class="row shadow justify-content-center">
                <div class="col-md-auto">
                  <button @click="showModal(updRes)" class="btn btn-info btn-sm">
                    <i class="fas fa-edit"></i> Edit
                  </button>
                </div>
                <div class="col-md-auto">
                  <button @click="delResource(updRes.id)" class="btn btn-danger btn-sm">
              ...
              <div class="card-body shadow text-dark">
                <div class="row">
                  <div class="col-md-8 p-0">
                    <ul>
                      <li>
                        <b class="bg-secondary px-1">Title:</b>
                        {{updRes.res_title}}
                      </li>
                      <li>
                        <b class="bg-secondary px-1">Subjec:</b>
                        {{updRes.res_subject}}
                      </li>
                      <li>
                        <b class="bg-secondary px-1">Author:</b>
                        {{updRes.res_author}}
                      </li>
                      <li v-if="updRes.res_type">
                        <b class="bg-secondary px-1">Type:</b>
                        {{updRes.res_type.res_type}}
                      </li>
                      <li v-if="updRes.res_type">
                        <b class="bg-secondary px-1">Department:</b>
                        <small>{{updRes.res_department.department}}</small>
                      </li>
                      <li>
                        <b class="bg-secondary px-1">Uploaded:</b>
                        {{updRes.created_at|myDate}}
                      </li>
                    </ul>
                  </div>
                  <div class="col-md-4">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    <select
                      type="text"
                      v-model="form.res_department_id"
...
export default {
  ...
  methods: {
    ...
    loadDeptAndTypes() {
      if (!this.departments) {
        axios.get(`library/departments`).then(res => {
          this.departments = res.data;
        });
      }
      if (!this.types) {
        axios.get(`library/res_type_all`).then(res => {
          this.types = res.data;
        });
      }
    },
    ...
    getImagePreviews() {
      for (let i = 0; i < this.form.res_files.length; i++) {
        if (/\.(jpe?g|png|gif)$/i.test(this.form.res_files[i].name)) {
          this.form.res_files[i].loadimg = URL.createObjectURL(
            this.form.res_files[i]
          );
        } else {
    ...
    editResource() {
      this.$Progress.start();
      this.form.put(`library/resource/${this.form.id}`).then(res => {
        this.updateDataTable();
    ...
    delResource(id) {
      Swal.fire({
        ...
        if (result.value) {
          axios.delete(`library/resource/${id}`).then(res => {
            this.updRes = res.data;
            this.updateDataTable();
            $("#resourceModal").modal("hide");
            $("#viewResource").modal("hide");
            Swal.fire("Deleted!", "Resource type has been deleted.", "success");
          });
        }
      });
    }
