        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    ...
                    url="library/res_type"></data-table>
                </div>
            </div>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="container-fluid">   
                            ...
                            <div class="card-body shadow text-dark">
                                <ul>
                                    <li>Folder: {{res_type.res_type}}</li>
                                    <li>Total: {{numOfRes}}</li>
                                    <li>Created: {{res_type.created_at|myDate}}</li>
                                </ul>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        viewType(typ) {
            this.res_type = typ;
            axios.get(`library/res_count/${typ.id}`)
            .then(res => {
                this.numOfRes = res.data;
            });
            $('#viewType').modal('show');
        },
        newType() {
            this.$Progress.start();
            this.form.post(`library/res_type`)
            .then(res  => { 
        ...
        editType() {
            this.$Progress.start();
            this.form.put(`library/res_type/${this.form.id}`)
            .then(res  => { 
                this.res_type = res.data;
                this.updateDataTable();
                Toast.fire({
        ...
        delType(id){
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete(`library/res_type/${id}`)
                    .then(res => {
                        this.res_type = res.data;
                        this.updateDataTable();
                        $('#viewType').modal('hide');
